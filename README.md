...
- **React 19 + TypeScript** фронтенд с современным UI
...
- **ОС:** Linux (протестировано на Fedora)
...
Если используется кастомный путь, настройте переменную `QWEN_PATH`.
...
# Лимиты
export MAX_CONTEXT_CHARS="120000"
export TOOL_RESULT_MAX_CHARS="4000"

# CORS
export ALLOWED_ORIGINS="http://localhost:10310,http://127.0.0.1:10310"
```
...
```bash
uvicorn server:app --host 0.0.0.0 --port 10310 --workers 4
```
...
|------------|----------|--------------|
| `QWEN_PATH` | Путь к исполняемому файлу qwen CLI | `/home/andrew/.nvm/versions/node/v22.12.0/bin/qwen` |
| `MCP_PYTHON` | Путь к Python для MCP сервера | `sys.executable` (текущий Python) |
| `MAX_CONTEXT_CHARS` | Максимальный размер контекста (0 = без лимита) | `0` |
| `TOOL_RESULT_MAX_CHARS` | Максимальный размер результата инструмента (0 = без лимита) | `0` |
| `ALLOWED_ORIGINS` | Разрешённые CORS origin (через запятую) | `http://localhost:10310,http://127.0.0.1:10310` |
...
- `user_id` (TEXT) — ID пользователя (OAuth)
...
**Индексы:**
- `idx_sessions_user_id` — для поиска по user_id
- `idx_messages_session_id` — для поиска сообщений сессии
- `idx_memory_session_id` — для поиска памяти сессии
